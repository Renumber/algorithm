...
# 풀이
큰 사각형에서 작은 사각형을 빼 넓이를 구한다.   
ㄱ모양을 회전시킨 모양이기 때문에 가장 큰 사각형 중 반 시계방향 기준 먼저나온 변(예제에서는 4, 50)으로부터 3, 4번째 뒤에 나오는 변(예제에서는 1,60 / 3,20)을 곱하면 작은 사각형의 넓이를 구할 수 있다.
이렇게 구한 큰 사각형의 넓이에서 작은 사각형의 넓이를 뺀 다음 k를 곱하면 참외의 개수를 구할 수 있다.
